/**
 * Domain primitive types. Field names mirror PROJECT.md <DATA_MODEL> and the
 * SQLite schema in schema.sql. These are p_code types describing q_code data
 * — never the q_code content itself.
 */
...
export interface Project {
  ...
  created_at: string;
  updated_at: string;
  /** True for a Project imported for ComparisonView — read-only, hidden from the main dashboard. */
  is_comparison_source: 0 | 1;
  /** Single highlight color used for every q_code's CodedExcerpts in this Project — see PROJECT.md UI note. */
  highlight_color: string;
  /** User-controlled dashboard ordering — lower sorts first. */
  sort_order: number;
  /** The Codebook version currently live for coding — see codebooksService/codebookVersionsService. */
...
/** Transcript minus raw_text — for list views that only ever show title/file_name, never the body. */
...
export interface QualitativeCode {
  id: string;
  codebook_id: string;
  /** Interview Question this code was created under, if any — see schema.sql for scoping rules. */
